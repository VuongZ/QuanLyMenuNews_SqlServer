...
public class UpdateMenuNewsItemRequestValidator : AbstractValidator<UpdateMenuNewsItemRequest>
{
    ...
    {
        RuleFor(x => x.Id).GreaterThan(0).GreaterThan(0).WithMessage("Id News phải lớn hơn 0.");
        RuleFor(x => x.Title).NotEmpty().WithMessage("Tiêu đề News không được để trống.").MaximumLength(255).WithMessage("Tiêu đề News tối đa 255 ký tự.");
        RuleFor(x => x.Slug).NotEmpty().WithMessage("Slug News không được để trống.").MaximumLength(255).WithMessage("Slug News tối đa 255 ký tự.").Matches("^[a-zA-Z0-9-_]+$").WithMessage("Slug chỉ được chứa chữ, số, gạch ngang hoặc gạch dưới.");
